...
## Server-side repo tracking (target: v0.2.0)

Already scoped in full; see the design discussion preceding PR #8
merge. Short version:

- New D1 table `user_repos(user_id, fingerprint, first_seen_at, last_seen_at)`
  with `PRIMARY KEY(user_id, fingerprint)` + `ON DELETE CASCADE FROM users`.
- Worker routes: `POST /v1/account/repos` (atomic INSERT ... WHERE
  count < max), `GET /v1/account/repos` (list), `DELETE
  /v1/account/repos/:fingerprint` (free slot).
- CLI: `recon repos list` / `recon repos remove` subcommands. Canonical
  absolute path → SHA256 on the client; Worker only sees the hex.
- First-run migration: bulk-register entries from
  `~/.config/recon/repos.json` on v0.2.0 first run; rename the local
  file to `.migrated-v0.2`.
- Framing in CHANGELOG: "raises the bar for casual bypass," NOT "hard
  gate" — a patched binary that skips the POST still bypasses. True
  hard gate needs license re-validation to carry the active repo list;
  that's v0.4+ work.

Tradeoff documented: fingerprint is path-based, so **moving a repo
directory burns a slot**. Git-remote dedup is possible later but adds
complexity (repos without remotes, forks, submodules).
